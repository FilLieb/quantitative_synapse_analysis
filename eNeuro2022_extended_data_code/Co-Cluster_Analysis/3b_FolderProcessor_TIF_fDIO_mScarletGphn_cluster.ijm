...
function processFiles(directory) {
	...
	for (file = 0; file < fileList.length; file++) {
		...
		} else if (endsWith(fileList[file], "/") && !matches(fileList[file], ".*mScarlet-Gphn_Analysis_.*")) {
...
Every individual ROI (mScarlet-Gphn) cluster will be classified as "negative" or "positive" for vGAT, mEGFP, or Soma, based on their overlap with the respective masks. 
...
function analyzeImage() {	
	...
	//classification of individual clusters
		...
		getStatistics(area, mean, min, max);
		if (mean > 1) {
			setResult("mEGFPGphn", m, "positive");
		} else {
			setResult("mEGFPGphn", m, "negative");
		}
