...
If these files are detected, a subdirectory with the name "mScarlet-Gphn_Analysis_" will be created (or "alpha2_Analysis_" + index, if the file name already exists). 
...
function processFiles(directory) {
	...
	outputDirName = directory + "alpha2_Analysis_";//

	folderCount = 1;
	
	while (File.exists(outputDirName)) {
		print(outputDirName + " exists");
		outputDirName = directory + "alpha2_Analysis_" + folderCount; //
		folderCount++;
	}
	...
	for (file = 0; file < fileList.length; file++) {
		...
		} else if (endsWith(fileList[file], "/") && !matches(fileList[file], ".*alpha2_Analysis_.*")) { //
...
function analyzeImage() {	
	...
	for (m = 0; m < roiNumber; m++) {
		...
		getStatistics(area, mean, min, max);
		if (mean > 1) {
			setResult("alpha1", m, "positive");
		} else {
			setResult("alpha1", m, "negative");
		}
