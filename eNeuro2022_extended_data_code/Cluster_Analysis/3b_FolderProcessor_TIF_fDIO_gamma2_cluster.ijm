...
If these files are detected, a subdirectory with the name "gamma2_Analysis_" will be created (or "gamma2_Analysis_" + index, if the file name already exists). 
...
function processFiles(directory) {
	...
	outputDirName = directory + "gamma2_Analysis_";//name of the newly created subfolder

	folderCount = 1;
	
	while (File.exists(outputDirName)) {
		print(outputDirName + " exists");
		outputDirName = directory + "gamma2_Analysis_" + folderCount; //when a subfolder from a previous analysis exists, a new folder with underscore and index will be created. 
		folderCount++;
	}
...
/*
analyzeImage function will use the previously generated gamma2 mask (gamma2.tif) and with the imagej Analyze Particles algorithm, individual rois will be created.
The distance between the center of these ROIs (gamma2 clusters) and the center of the soma will be measured.
...
Every individual ROI (gamma2) cluster will be classified as "negative" or "positive" for vGAT, mScarlet-Gphn, or Soma, based on their overlap with the respective masks. 

Date, Cell, and Condition will be read from folder and subfolder organization.

Data will be written in an organized "semi-tidy" table (one line per gamma2 cluster). 
...
The RoiManager will be saved as "RoiSetgamma2Clusters.zip"
*/
...
function analyzeImage() {	
	...
	//save
	roiManager("Save", outputDirPath+"RoiSetgamma2Clusters.zip");
	selectWindow("ROI Manager");
    run("Close");
