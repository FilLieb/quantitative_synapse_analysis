...
function maskCell() { 
	...
	run("Manual Threshold...", "min=2000 max=65535");
	run("Convert to Mask");
	run("Options...", "iterations=4 count=1 black do=Dilate");
	run("Analyze Particles...", "size=250-Infinity show=Masks");//350
	rename("Cell.tif");
...
function maskSoma() { 
	...
	run("Enhance Contrast...", "saturated=0.1 normalize");	//0.1
	run("Gaussian Blur...", "sigma=2");
	run("Manual Threshold...", "min=25000 max=65535");
	run("Convert to Mask");
	run("Options...", "iterations=4 count=1 black do=Dilate");
...
function maskgamma2() {
	...
	run("Subtract Background...", "rolling=30");
	run("Gaussian Blur...", "sigma=2");
	run("Manual Threshold...", "min=2500 max=65535");
	run("Convert to Mask");
	run("Watershed");
	run("Analyze Particles...", "size=0.01-Infinity show=Masks");
	rename("gamma2.tif");
...
function maskMScarlet() {
	...
	run("Subtract Background...", "rolling=30");
	run("Gaussian Blur...", "sigma=2");
	run("Manual Threshold...", "min=3500 max=65535");
	run("Convert to Mask");
	run("Watershed");
	run("Analyze Particles...", "size=0.01-Infinity show=Masks");
	rename("Gphn.tif");
